...
void setup()
{
  ...
  // Set open fasle (default)
  Serial.printf("Set open false", Firebase.setInt(fbdo, "/product/window/env/open", false) ? "ok" : fbdo.errorReason().c_str());
  pinMode(A0, INPUT);
}
...
void loop() { //열렸는지 안열였는지 보냄
  ...
  if (digitalRead(SWITCH1) == LOW) {
    if (!open_status) {
      Serial.printf("Set open true... %s\n", Firebase.setInt(fbdo, "/product/window/env/open", true) ? "ok" : fbdo.errorReason().c_str());
      open_status = true;
    }
  } else {
    if (open_status) {
